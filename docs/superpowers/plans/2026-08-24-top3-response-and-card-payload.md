...
- [ ] 将 Prompt 改为有 N 个候选就必须输出 N 个编号商品，每项含名称、价格、匹配理由。
- [ ] 将回复校验改为逐个候选验证；任一漏写则调用 `_template_answer(top)`。
- [ ] 保持 0、1、2 件候选的原有诚实兜底行为。
...
- [ ] 在 `_products_payload` 输出 `category`、`sub_category`、`skus`、`description` 和 `image_urls`。
- [ ] 保留 `product_id`，使现有卡片点击继续导航到 `/api/products/{product_id}`。
- [ ] 静态检查差异并以中文提交说明提交；按用户要求不自动执行测试套件。
